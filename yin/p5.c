...
static void performance_end(struct system_t *system)
{
    ...
    //double aqc = (double)system->total_queue_customer / system->total_event;
    //aqc = system->total_delay_time / 60000;
#ifdef __10000_CUSTOMER__
    double total = system->current_time;
#else
    double total = SIM_TIME;
#endif
    double aqc = system->area / total;
